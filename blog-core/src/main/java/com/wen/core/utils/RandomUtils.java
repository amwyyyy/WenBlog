...
public class RandomUtils {
   ...
   * 生成随机字符串
   *
   * @param charSet 字符串中允许出现的字符
   * @param len     字符串的长度
   * @return 随机字符串
   */
  public static String getString(String charSet, int len) {
    char[] chars = new char[len];
    for (int i = 0; i < len; ++i) {
      int r = RANDOM.nextInt(charSet.length());
      chars[i] = charSet.charAt(r);
    }
  ...
  public static String getString(int len) {
    return getString(RandomUtils.DEFAULT_RAND_CHAR_SET, len);
  }
